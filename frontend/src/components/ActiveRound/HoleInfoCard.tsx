...
import { getHoleDistanceMeters } from '../../utils/teeDistances';
...
export const HoleInfoCard: React.FC<HoleInfoCardProps> = ({
  ...
  const teeColors: { key: TeeColor; name: string; dotBg: string }[] = [
    ...
    { key: 'yellow', name: 'Yellow', dotBg: 'bg-[#E6CC7A]' },
        ...
        <div className="text-right">
          ...
          <div className="flex items-baseline justify-end gap-1">
            <span className="text-2xl sm:text-3xl font-bold text-[#E6CC7A] font-mono">
              {getHoleDistanceMeters(hole, activeTee) || getHoleDistanceMeters(hole, 'white') || 320}
            </span>
            <span className="text-xs text-[#CCD7BE] font-semibold">m</span>
          </div>
        </div>

      </div>

      {/* All tees distance strip */}
          ...
          {teeColors.map((t) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-mono text-xs font-semibold">{getHoleDistanceMeters(hole, t.key) || '-'}</span>
              </div>
            );
          })}
        </div>
